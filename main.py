...
async def welcome_endpoint(request: Request):
    """
    Returns the welcome JSON payload if requested by a script/CLI tool, otherwise serves the HTML UI.
    """
    accept = request.headers.get("accept", "").lower()
    user_agent = request.headers.get("user-agent", "").lower()
    
    # If explicitly requesting JSON, or using a CLI tool/script library:
    if "application/json" in accept or any(tool in user_agent for tool in ["curl", "python", "urllib", "httpx", "postman"]):
        return JSONResponse(content={
            ...
            "interactive_ui": "/ui",
            "endpoints": {
                "GET /": "Welcome message / Interactive UI",
    ...
    # Default to serving the HTML UI
    index_path = find_index_html()
    if index_path:
        with open(index_path, "r", encoding="utf-8") as f:
            return HTMLResponse(content=f.read())
            
    return HTMLResponse(content="<h1>Documind UI is loading...</h1>")
